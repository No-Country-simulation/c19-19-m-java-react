...
export default function ButtonNav({ children, active, to }) {
  ...
  return (
    <>
      <Link
        href={to}
        className={`group hover:transition-all hover:bg-customGreen hover:border-2 hover:border-solid hover:border-customGray h-8 w-28 xl:w-40 rounded-2xl flex justify-center items-center ${stateClass}`}
      >
        {active ? (
          <MenuText
